...
daemonpkgs = ./cmd/fftchaind
clientpkgs = ./cmd/fftchainc
...
configpkg = github.com/threefoldtech/FFTChain/pkg/config
ldflagsversion = -X $(configpkg).rawVersion=$(fullversion)

stdoutput = $(GOPATH)/bin
daemonbin = $(stdoutput)/fftchaind
clientbin = $(stdoutput)/fftchainc
