...
<head>
	<title>FFT Faucet</title>
</head>
<body>
	<div align="center">
		<h1 style="margin-top:3em">fftchain {{.ChainNetwork}} faucet</h1>
		...
		<h3>Request %[1]d FFT by entering your address below and submitting the form.</h3>
		<form action="/request/tokens" method="POST">
			<div>Address: <input type="text" size="78" name="uh"></div>
			<br>
			<div><input type="submit" value="Request %[1]d FFT" style="width:20em;height:2em;font-weight:bold;font-size:1em;"></div>
		</form>

		
	
		<div style="margin-top:50px;"><small>fftchain faucet</small></div>
	</div>
</body>
`, coinsToGive))
...
<body>
	<div align="center">
		<h1>%d FFT succesfully transferred on fftchain's {{.ChainNetwork}} to {{.Address}}</h1>
